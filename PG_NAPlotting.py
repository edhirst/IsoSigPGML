'''Snappy OrientedClosedCensus Network Analysis 
The datafile imported lists data :
    (census index, (#triangulations, density), degree distribution, 
    clustering (tri,squ), min cycle basis info, wiener index, 
    EV centrality (max,range)), NumTetrahedra (index of min, min, unique distribution)
'''
import numpy as np
...
for data_idx in range(len(depths)):
    print(f'Depth: {depths[data_idx]}, Moves: {move_sets[data_idx]}')
    '''
    #Return frequency of each cycle length
    basis_lengths = [sum([cycle_size[1] for cycle_size in manifold]) for manifold in cycles[data_idx]] #...total *number* of cycles in each basis
    #basis_uniquelengths = np.array(list(map(len,cycles[data_idx]))) #...number of unique cycle lengths in each basis
    basis_lengths_unique, basis_length_counts = np.unique(basis_lengths,return_counts=True)
    print(f'Basis Lengths: {basis_lengths_unique.tolist()}, counts: {basis_length_counts.tolist()}')
    #Average number of cycles per manifold
    print(f'Average basis length: {np.mean(basis_lengths)}')
    '''
